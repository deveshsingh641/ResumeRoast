...
export default function PaperMockup({
  ...
  if (issues && issues.length > 0) {
    if (issues[0]) {
      ...
      finalBullet1Tag =
        issues[0].badge_label?.trim() || getHinglishTag(issues[0].category);
    ...
    if (issues[1]) {
      ...
      finalBullet2Tag =
        issues[1].badge_label?.trim() || getHinglishTag(issues[1].category);
    ...
    if (issues[2]) {
      ...
      finalBullet3Tag =
        issues[2].badge_label?.trim() || getHinglishTag(issues[2].category);
  ...
  useEffect(() => {
    ...
    if (
      controlledMarkStep !== undefined ||
      controlledPaperSettled !== undefined
    ) {
      return;
    }
  ...
  const getHeatmapColor = (cat: string) => {
    switch (cat) {
      ...
      case "typo":
        return {
          color: "#E8422D",
          bg: "rgba(232, 66, 45, 0.12)",
          label: "CRITICAL [RECRUITER DROP-OFF]",
        };
      case "formatting":
      case "length":
        return {
          ...
          label: "MODERATE [WASTED REAL ESTATE]",
        };
      default:
        return {
          color: "#7FA65C",
          bg: "rgba(127, 166, 92, 0.12)",
          label: "OPTIMAL [CLEAR IMPACT]",
        };
    }
  };
      ...
      >
        ...
        <div
          className={`relative border-b border-black/10 pb-4 mb-6 text-left ${
            xRayMode
              ? "p-2 rounded bg-emerald-500/10 border border-emerald-600/30"
              : ""
          }`}
        >
          ...
          {items.map((item, idx) => {
            ...
            return (
              <div key={item.id} className="relative flex items-start gap-2">
                <span className="font-mono text-xs text-ink/50 select-none mt-0.5">
                  •
                </span>
