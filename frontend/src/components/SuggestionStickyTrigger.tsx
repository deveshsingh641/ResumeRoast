...
export default function SuggestionStickyTrigger() {
  ...
  useEffect(() => {
    ...
    try {
      const seen = localStorage.getItem('resumeroast_has_seen_result') === 'true'
      const hasRecentRoast = Boolean(localStorage.getItem('resumeroast_last_id'))
    ...
    return () => window.removeEventListener('open-suggestion-box', handleOpenEvent)
  }, [])
