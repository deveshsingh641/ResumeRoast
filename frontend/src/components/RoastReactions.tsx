...
export default function RoastReactions({
  roastId,
  className = "",
}: RoastReactionsProps) {
  ...
  useEffect(() => {
    ...
    const fetchReactions = async () => {
      try {
        const { data } = await axios.get(`/api/roast/${roastId}/reactions`, {
          timeout: 8000,
        });
        if (data?.reactions) {
  ...
  const handleReact = async (key: EmojiType) => {
    ...
    const totalUserReacts = Object.values(userReacted).reduce(
      (a, b) => a + b,
      0,
    );
      ...
      <div className="flex items-center gap-2">
        {REACTION_CONFIGS.map((item) => {
          ...
          return (
            ...
            >
              <span
                className={`text-base leading-none transition-transform duration-150 ${isBouncing ? "scale-125" : "group-hover:scale-115"}`}
              >
                {item.symbol}
              </span>
